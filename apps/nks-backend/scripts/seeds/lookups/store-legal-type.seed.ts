...
export async function seedStoreLegalTypes(db: Db) {
  const [type] = await db.select({ id: lookupType.id }).from(lookupType).where(eq(lookupType.code, 'STORE_LEGAL_TYPE')).limit(1);
  if (!type) throw new Error('lookup_type STORE_LEGAL_TYPE not seeded — run seedLookupTypes first');
