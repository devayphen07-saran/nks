...
export const commodityCodes = pgTable(
  'commodity_codes',
  {
    ...
    // Classification-level default rate — used as a template/hint when creating a new
    // tax_rate_master record for a store. NOT the authoritative rate for transactions.
    // Authoritative rate:  SELECT * FROM tax_rate_master WHERE store_fk = ? AND commodity_code_fk = ?
    // Examples: India GST (5, 12, 18, 28), UK VAT (20, 5, 0)
