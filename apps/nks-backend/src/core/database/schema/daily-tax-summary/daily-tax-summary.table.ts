 ...
 *
 * Example (India GST):
 *   Country: India, Store: 1, Date: 2024-03-27, Tax Rate: 18%
 *   Total Taxable: 50,000 → Component1 (CGST): 4,500, Component2 (SGST): 4,500, Total: 9,000
 *
 * Example (UK VAT):
 *   Country: UK, Store: 1, Date: 2024-03-27, Tax Rate: 20%
 *   Total Taxable: 50,000 → Tax Collected: 10,000
 */
export const dailyTaxSummary = pgTable(
  'daily_tax_summary',
  {
    ...
    // Sum of all component 1 amounts (e.g., CGST for India, NULL for single-rate countries)
    totalComponent1Amount: numeric('total_component1_amount', {
      precision: 15,
      scale: 2,
    })
      .notNull()
    ...
    // Sum of all component 2 amounts (e.g., SGST for India, NULL for single-rate countries)
    totalComponent2Amount: numeric('total_component2_amount', {
      precision: 15,
      scale: 2,
    })
      .notNull()
    ...
    // Sum of all component 3 amounts (e.g., IGST for India, NULL for other countries)
    totalComponent3Amount: numeric('total_component3_amount', {
      precision: 15,
      scale: 2,
    }).default('0'),

    // Sum of all additional/cess amounts (e.g., cess for India, NULL for other countries)
    totalAdditionalAmount: numeric('total_additional_amount', {
      precision: 15,
      scale: 2,
    }).default('0'),

    // Total tax collected = component1 + component2 + component3 + additional
  ...
  (table) => [
    ...
    check(
      ...
      sql`total_tax_collected = total_component1_amount + total_component2_amount + total_component3_amount + COALESCE(total_additional_amount, 0)`,
    ),

    // No negative amounts
    check(
      'daily_tax_summary_amounts_positive_chk',
      sql`total_taxable_amount >= 0 AND total_component1_amount >= 0 AND total_component2_amount >= 0 AND total_component3_amount >= 0 AND total_tax_collected >= 0`,
    ),
