...
export function analyzeStorageUsage(authResponse: AuthResponse): {
  ...
  if (sizeBytes > SECURESTORE_CRITICAL_BYTES) {
    status = "critical";
    message =
      `CRITICAL: Auth data is ${usagePercent}% of SecureStore limit. ` +
      `Roles: ${authResponse.access?.roles?.length ?? 0}, ` +
      `Stores: ${new Set((authResponse.access?.roles ?? []).map((r: any) => r.storeId)).size}`;
