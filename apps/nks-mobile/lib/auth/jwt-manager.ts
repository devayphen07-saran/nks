...
export const JWTManager = {
  ...
  async cacheJWKS(baseUrl: string): Promise<boolean> {
    ...
    try {
      ...
      // Backend wraps all responses in { success, data } via TransformInterceptor
      const jwks = body?.data ?? body;
      ...
      if (!_jwksCache) {
        throw new Error("JWKS_UNAVAILABLE_OFFLINE: No cached key — go online to sync.");
      }
      if (Date.now() - _jwksCache.cachedAt > JWKS_STALE_LIMIT_MS) {
        log.error(
          "JWKS cache is older than 7 days and network is unavailable. " +
          "Cannot verify offline token — rejecting operation.",
        );
        throw new Error("JWKS_STALE_OFFLINE: Cached key too old — go online to sync.");
      }
      return true;
    }
  },
  ...
  /**
   * Returns true if the JWKS cache exists and is within the 7-day stale limit.
   * Use this in write-guards to block offline writes when verification is impossible.
   */
  isJwksFresh(): boolean {
    if (!_jwksCache) return false;
    return Date.now() - _jwksCache.cachedAt <= JWKS_STALE_LIMIT_MS;
  },
